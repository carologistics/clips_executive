    ...
    rcl_interfaces::msg::ParameterDescriptor())
{
  if (!node->has_parameter(param_name)) {
    RCLCPP_DEBUG(rclcpp::get_logger("cx_utils"), "Creating parameter: %s", param_name.c_str());
...
std::string get_plugin_type_param(NodeT node, const std::string & plugin_name)
{
  ...
  try {
    if (!node->get_parameter(plugin_name + ".plugin", plugin_type)) {
      ...
      throw std::runtime_error(std::string("'plugin' param not defined for '") + plugin_name + "'");
    }
...
/// Fetch absolute paths of files and make sure they exist
/* Resolves a list of files by checking absolute paths and package share directories.
 * @param files_in List of file paths to resolve (absolute or relative).
 * @param share_dirs List of ROS package names to search in for relative paths.
 * @param files_out Output vector where resolved absolute paths are written.
 * @throws std::runtime_error if a file cannot be resolved via any of the provided paths.
 * @note Packages in share_dirs that are not found in the ament index are skipped
 */
void resolve_files(
  const std::vector<std::string> & files_in, const std::vector<std::string> & share_dirs,
