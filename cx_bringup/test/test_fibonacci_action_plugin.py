...
generate_test_description = pytest.mark.launch_test(
    launch_testing.markers.keep_alive(make_generate_test_description('fibonacci_action.yaml'))
)
...
class TestFibonacciActionPluginOutput(unittest.TestCase):
    ...
    def test_both_environments_created(self, cx_node, proc_output):
        ...
        wait_for_output(
            proc_output, 'Initialized new CLIPS environment: cx_fibonacci_client', cx_node
        )
    ...
    def test_both_server_and_client_see_final_result(self, cx_node, proc_output):
        ...
        wait_for_output(
            proc_output,
            '[cx_fibonacci_client] [INFO] Final fibonacci sequence: (0 1 1 2 3 5)',
            cx_node,
            timeout=60.0,
        )
    ...
    def test_cancel_confirmed_by_client(self, cx_node, proc_output):
        wait_for_output(
            proc_output,
            '[cx_fibonacci_client] [INFO] Canceled fibonacci sequence: (0 1 1 2 3 5 8)',
            cx_node,
            timeout=60.0,
        )
