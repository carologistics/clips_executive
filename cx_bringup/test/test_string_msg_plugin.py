...
def generate_test_description():
    manager_config_file = PathJoinSubstitution(
        [
            ...
            TextSubstitution(text='params/plugin_examples/string_msg.yaml'),
        ]
    )
...
class TestRosMsgsPluginOutput(unittest.TestCase):
    ...
    def test_publisher_fact_asserted(self, cx_node, proc_output):
        wait_for_fact_asserted(
            proc_output, '(std-msgs-string-publisher (topic "ros_cx_out"))', cx_node
        )

    def test_subscriber_fact_asserted(self, cx_node, proc_output):
        wait_for_fact_asserted(
            proc_output, '(std-msgs-string-subscription (topic "ros_cx_in"))', cx_node
        )
    ...
    def test_message_received_and_hello_world_published(self, cx_node, proc_output):
        ...
        wait_for_fact_asserted(
            proc_output, '(std-msgs-string-message (topic "ros_cx_in")', cx_node, timeout=60.0
        )
        wait_for_rule_fire(proc_output, 'string-publisher-recv-and-answer', cx_node, timeout=60.0)
