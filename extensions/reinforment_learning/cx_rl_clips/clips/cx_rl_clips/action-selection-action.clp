...
=>
  (if (not (cx-rl-interfaces-action-selection-server-goal-handle-is-canceling ?ptr)) then
      ...
      (bind ?action_id (cx-rl-interfaces-action-selection-goal-get-field ?goal "action_id"))
      (bind ?uuid (cx-rl-interfaces-action-selection-server-goal-handle-get-goal-id ?ptr))
      (assert (rl-ros-action-meta-action-selection (node ?node-name) (uuid ?uuid) (action-id (sym-cat ?action_id))))
...
(defrule cx-rl-action-selection-abort
  ...
  (test (str-index ?node-name ?server))
  ?as <- (rl-ros-action-meta-action-selection (node ?node-name) (abort-action TRUE) (action-id ?action_id) (uuid ?uuid&:(eq ?uuid (cx-rl-interfaces-action-selection-server-goal-handle-get-goal-id ?ptr))))
=>
  (printout ?*CX-RL-LOG-LEVEL* "ResetEnv: Aborting action " ?action_id crlf)
  (bind ?result (cx-rl-interfaces-action-selection-result-create))
  (cx-rl-interfaces-action-selection-result-set-field ?result "action_id" (str-cat ?action_id))
...
=>
  ...
  (bind ?result (cx-rl-interfaces-action-selection-result-create))
  (cx-rl-interfaces-action-selection-result-set-field ?result "action_id" (str-cat ?action-id))
