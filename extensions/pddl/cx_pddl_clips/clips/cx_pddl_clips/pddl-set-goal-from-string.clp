...
(defrule pddl-set-goal-from-string-request
  (declare (salience ?*PRIORITY-PDDL-SET-GOALS*))
  (pddl-set-goal-from-string (instance ?instance) (goal ?goal) (goal-description ?goal-desc) (state PENDING))
  (pddl-manager (node ?node))
  ?pi-f <- (pddl-instance (name ?instance) (state LOADED) (busy-with FALSE))
  (ros-msgs-client (service ?s&:(eq ?s (str-cat ?node "/set_goal_from_string"))) (type ?type))
  ...
  (ros-msgs-set-field ?new-req "goal_string" ?goal-desc)
...
(defrule pddl-set-goal-from-string-response-received
" Get response, read it and delete."
  ?set-goal-from-string-f <- (pddl-set-goal-from-string (instance ?instance) (goal ?goal) (state PENDING))
  (pddl-manager (node ?node))
  ?pi-f <- (pddl-instance (name ?instance) (busy-with SET-GOALS))
  (ros-msgs-client (service ?s&:(eq ?s (str-cat ?node "/set_goal_from_string"))) (type ?type))
...
=>
  ...
  (if ?success then
    (modify ?set-goal-from-string-f (state DONE))
   else
    (modify ?set-goal-from-string-f (state ERROR) (error ?error))
  )
